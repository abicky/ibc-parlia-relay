...
async function main() {
    ...
    // deploy client
    const parliaClient = await deploy(deployer, "ParliaClient", [ibcHandler.target])
    await ibcHandler.registerClient(ParliaClientType, parliaClient.target).then(tx => tx.wait());
